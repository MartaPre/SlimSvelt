...
from .serializers import DietarySerializer


class DietaryListView(
  ...
  def get(self, request, id=None):
    if id:
      ...
      read_serializer = DietarySerializer(queryset, many=True)
  ...
  def post(self, request):
    ...
    create_serializer = DietarySerializer(data=request.data)
    ...
    if create_serializer.is_valid():
      ...
      dietary_item_object = create_serializer.save()

      # Serialize the new todo item from a Python object to JSON format
      read_serializer = DietarySerializer(dietary_item_object)
  ...
  def put(self, request, id=None):
    try:
      # Check if the todo item the user wants to update exists
      dietary_item = Dietary.objects.get(id=id)
    except Dietary.DoesNotExist:
      # If the todo item does not exist, return an error response
      return Response({'errors': 'This dietary item does not exist.'}, status=400)

    # If the todo item does exists, use the serializer to validate the updated data
    update_serializer = DietarySerializer(dietary_item, data=request.data)
    ...
    if update_serializer.is_valid():
      ...
      dietary_item_object = update_serializer.save()

      # Serialize the todo item from Python object to JSON format
      read_serializer = DietarySerializer(todo_item_object)
