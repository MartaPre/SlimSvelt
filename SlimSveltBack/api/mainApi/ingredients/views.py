...
class IngredientsListView(
  ...
  def get(self, request, id=None):
    if id:
      ...
      try:
        ...
        queryset = Ingredients.objects.get(id=id)
      except Ingredients.DoesNotExist:
        # If the todo item does not exist, return an error response
        return Response({'errors': 'This Ingredients item does not exist.'}, status=400)

      # Serialize todo item from Django queryset object to JSON formatted data
      read_serializer = IngredientsSerializer(queryset)
      ...
      read_serializer = IngredientsSerializer(queryset, many=True)
  ...
  def post(self, request):
    ...
    create_serializer = IngredientsSerializer(data=request.data)
    ...
    if create_serializer.is_valid():
      ...
      ingredients_item_object = create_serializer.save()

      # Serialize the new todo item from a Python object to JSON format
      read_serializer = IngredientsSerializer(ingredients_item_object)
  ...
  def put(self, request, id=None):
    try:
      # Check if the todo item the user wants to update exists
      ingredients_item = Ingredients.objects.get(id=id)
    except Ingredients.DoesNotExist:
      # If the todo item does not exist, return an error response
      return Response({'errors': 'This Ingredients item does not exist.'}, status=400)

    # If the todo item does exists, use the serializer to validate the updated data
    update_serializer = IngredientsSerializer(ingredients_item, data=request.data)
    ...
    if update_serializer.is_valid():
      ...
      ingredients_item_object = update_serializer.save()
  ...
  def delete(self, request, id=None):
    try:
      # Check if the todo item the user wants to update exists
      ingredients_item = Todo.objects.get(id=id)
    except Ingredients.DoesNotExist:
      # If the todo item does not exist, return an error response
      return Response({'errors': 'This Ingredients item does not exist.'}, status=400)

    # Delete the chosen todo item from the database
    Ingredients_item.delete()
