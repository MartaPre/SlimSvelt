...
from .models import TrainningDiary
from .serializers import TrainningDiarySerializer


class TrainningDiaryListView(
  ...
  def get(self, request, id=None):
    if id:
      ...
      try:
        ...
        queryset = TrainningDiary.objects.get(id=id)
      except TrainningDiary.DoesNotExist:
        # If the todo item does not exist, return an error response
        return Response({'errors': 'This TrainningDiary item does not exist.'}, status=400)

      # Serialize todo item from Django queryset object to JSON formatted data
      read_serializer = TrainningDiarySerializer(queryset)

    else:
      # Get all todo items from the database using Django's model ORM
      queryset = TrainningDiary.objects.all()

      # Serialize list of todos item from Django queryset object to JSON formatted data
      read_serializer = TrainningDiarySerializer(queryset, many=True)
  ...
  def post(self, request):
    ...
    create_serializer = TrainningDiarySerializer(data=request.data)
    ...
    if create_serializer.is_valid():
      ...
      trainning_diary_item_object = create_serializer.save()

      # Serialize the new todo item from a Python object to JSON format
      read_serializer = TrainningDiarySerializer(trainning_diary_item_object)
  ...
  def put(self, request, id=None):
    try:
      # Check if the todo item the user wants to update exists
      trainning_diary_item = TrainningDiary.objects.get(id=id)
    except TrainningDiary.DoesNotExist:
      # If the todo item does not exist, return an error response
      return Response({'errors': 'This trainning_diary item does not exist.'}, status=400)

    # If the todo item does exists, use the serializer to validate the updated data
    update_serializer = TrainningDiarySerializer(trainning_diary_item, data=request.data)
    ...
    if update_serializer.is_valid():
      ...
      trainning_diary_item_object = update_serializer.save()

      # Serialize the todo item from Python object to JSON format
      read_serializer = TrainningDiarySerializer(todo_item_object)
